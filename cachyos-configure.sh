...
# Usage: ./cachyos-configure.sh --profile [minimal|apps|dock|full]
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            echo "Usage: $0 --profile [minimal|apps|dock|full] [--dry-run]"
            echo ""
            echo "Profiles:"
...
case "$PROFILE" in
    ...
    full)
        ...
        ;;
    *)
        echo "ERROR: Unknown profile '${PROFILE}'. Valid: minimal, apps, dock, full" >&2
        exit 1
        ;;
esac
